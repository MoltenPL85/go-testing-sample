...
func Test_application_handlers(t *testing.T) {
	var theTests = []struct {
		...
		{"hame", "/", http.StatusOK, "/", http.StatusOK},
	...
	for _, e := range theTests {
		...
		if resp.Request.URL.Path != e.expectedURL {
			t.Errorf("%s: expected final url %s but got %s", e.name, e.expectedURL, resp.Request.URL.Path)
		}
...
func TestApp_renderWithBadTemplate(t *testing.T) {
	// set template path to the location with a bad template
